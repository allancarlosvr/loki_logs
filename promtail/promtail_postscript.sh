...
PROMTAIL_TMP_ZIP="/tmp/promtail.zip"
...
if command -v unzip >/dev/null 2>&1; then
  ...
  dnf --disablerepo=zabbix install -y unzip
fi
...
if [[ ! -x "${PROMTAIL_BIN}" ]]; then
  echo "==> Promtail binary not found. Installing version ${PROMTAIL_VERSION}..."

  if [[ -f "${PROMTAIL_TMP_ZIP}" ]]; then
    echo "==> Using pre-staged ZIP from ${PROMTAIL_TMP_ZIP}"
  else
    echo "==> No pre-staged ZIP found. Downloading from GitHub..."
    curl -fL -o "${PROMTAIL_TMP_ZIP}" "${PROMTAIL_ZIP_URL}"
  fi

  cd /tmp
  unzip -o "${PROMTAIL_TMP_ZIP}"
